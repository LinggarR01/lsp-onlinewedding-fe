...
function OrderTable({ data, loading, error, onEditClick, onDeleteClick }) {
  if (loading) {
    return (
      // TAILWIND LOADING STATE
      <div className="flex flex-col items-center justify-center p-8">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-red-600 mb-4"></div>
        <p className="text-gray-600 text-lg">Loading wedding orders...</p>
      </div>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      ...
      <div className="text-center p-8 bg-yellow-50 rounded-lg border border-yellow-200">
        ...
        <p className="text-yellow-600">No wedding order items found.</p>
      </div>
    );
  }
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {tableData.map((item, rowIndex) => (
              <tr
                ...
                <td className="px-6 py-4">
                  <div
                    className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium uppercase ${getStatusColor(
                      item.STATUS
                    )}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  ...
                  <button
                    onClick={() => onDeleteClick(item.id_order)}
